...
export default function UploadStatus({ appState }: UploadStatusProps) {
  ...
  return (
    <div role="status" aria-live="polite" className="flex items-center gap-2 text-sm">
      {status === 'uploading' && (
        <span className="flex items-center gap-2 text-slate-400 dark:text-zinc-400">
          <Spinner />
          Uploading
          {filename && (
            <span className="text-slate-400 dark:text-zinc-500 truncate max-w-[180px]">{filename}</span>
          )}
          …
        </span>
      )}
      ...
      {status === 'error' && (
        <span className="flex items-center gap-2 text-red-500 dark:text-red-400">
          <XIcon />
          <span className="truncate max-w-[220px]">{error ?? 'Something went wrong.'}</span>
        </span>
      )}
    </div>
  );
